...
Desktop may set `moduleRoot` to a separately installed Documents & data component. Conversion resolves that component’s renderer only after its installation marker is present; a missing component returns `unavailable` without loading native code. The default Web configuration continues to use its installed kit package.
