...
Strugend 构建保留 56px 顶栏，并默认打开 360px 的历史栏。用户可以调整宽度、关闭和重新打开。选择任务、调整窗口大小或打开右侧面板时，会保留用户选择的打开或关闭状态。重新加载后历史栏再次打开。侧栏占据独立列，不遮挡对话；已有右侧面板尺寸、全屏和键盘行为保持可用。
...
本插件在 root slot 中组合侧边栏、主内容和右栏。侧边栏宽度为 220～420px，默认为 240px，收起后保留 56px 控制栏；窗口宽度低于 1024px 时自动收起，打开右侧面板也会收起手动展开的侧边栏。右侧面板首次打开时使用视口宽度的 45%，之后保留用户的像素宽度偏好，上限为 70%。为给中栏保留 400px，框架先将右侧面板缩减至 300px，再报告空间不足，使占用方将其关闭，最后才进一步压缩中栏。拖动没有过渡延迟；右侧手柄在关闭或全屏时不显示。
...
`ctx.layout.toggleSidebar(true)` 可幂等打开侧栏以显示功能对话框；`false` 关闭侧栏，省略参数则切换状态。已展开时保留现有宽度。
