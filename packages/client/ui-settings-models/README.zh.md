...
在 Strugend 桌面组合中，连接页面优先显示提供方编辑器。首次设置允许选择任意可配置的提供方或兼容端点，然后将用户明确选择的模型保存为默认值。跳过设置后仍可使用应用；缺少凭据时会在开始回合前拒绝提交并保留草稿。可选 Decision、GitHub 和 Vercel 凭据位于折叠区域。Decision 运行方式和建议模式的更改使用设置版本，并显示本地内存限制。连接测试区分实际响应与已保存密钥的元数据。图谱记忆已禁用并显示即将推出。密钥仅可写入，环境凭据为只读。

Strugend 提供方设置先显示已配置的提供方目录和自定义端点，再显示凭据字段。稍后配置通过现有设置作用域持久化 `ui-onboarding.providerSetupDeferredVersion`，与欢迎通知独立；后续对话和应用重启不会重复提示。仍可手动进入连接设置。非回环浏览器仅在当前客户端生命周期内保留此选择。
