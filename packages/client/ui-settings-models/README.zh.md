...
在 Strugend 桌面组合中，智能服务页面提供 Core、可选的远程 Decision、GitHub 和 Vercel 凭据。Decision 默认使用自动模式，仅在内存允许时从远程回退到本地。物理内存小于 8 GiB 时本地模式不可用，页面显示限制原因。观察模式记录后台判断而不改变 Core 提示词；可选的辅助模式可提供已经完成的建议。运行方式和建议模式更改使用设置版本。折叠的其他 AI 提供方编辑器独立于 Decision 配置其他 Core 路由。连接测试将实际结果与已保存密钥的元数据分别显示。图谱记忆已禁用并显示即将推出。密钥仅可写入，环境凭据为只读。其他组合保留通用提供方编辑器。
