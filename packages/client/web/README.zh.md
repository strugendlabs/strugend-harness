...
Strugend 构建在启动时显示随应用打包的琥珀色折带标志和产品名称。加载时标志轻微浮动，在减少动态效果设置下保持静止。加载页遵循主题偏好。失败报告中的插件 ID 省略框架前缀，Host 仍保留底层诊断信息。
...
Agent OS 构建（`DSH_CLIENT_TITLE='Agent OS'`）使用应用内置的钛金属标志、轻微的呼吸动画和不显示百分比的加载条。插件词典到达前，启动文案根据浏览器语言显示英文或中文。页面遵循系统配色和减少动态效果设置，内联失败报告会停止动画，工作区就绪后立即交接。
