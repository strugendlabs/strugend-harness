...
Strugend 品牌插槽与导航栏、加载页共用随应用打包的琥珀色折带图片；该图片为装饰元素，不添加屏幕阅读器标签。
...
桌面组合按需打开技能、记忆和密码库；视频工作室禁用并标记为即将推出。对话分组组织会话，不改变工作区归属或持久化 ID。紧凑的工具入口和限制高度的分组滚动区为对话列表保留空间。可选工具提供按序设置选择和后续设置页面：决策支持可独立于编码模型跳过、安装、重试、取消或移除。下载大小来自发布目录；仅在下载和验证期间轮询状态。文档与数据工具为独立的可选下载。

桌面自动任务面板支持创建、预览、编辑、暂停和立即运行独立计划。最近运行卡片可打开结果任务；面板关闭后停止轮询。设置 → 更新提供下载检查和明确的安装偏好。[桌面参考](../../../apps/desktop/README.zh.md#scheduled-tasks-and-updates)说明执行和发行限制。
