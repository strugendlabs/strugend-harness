/** Product top bar and docked history sidebar, retaining the workspace slot. */
 ...
 * @returns Persistent top bar and user-collapsible history sidebar.
 */
export function MinimalNavigation({
  ...
  useEffect(() => {
    if (previouslyCollapsed.current && !collapsed) drawer.current?.focus()
    previouslyCollapsed.current = collapsed
  }, [collapsed])
  return (
    <>
      <header className={css.topbar} data-strugend-topbar>
        <button type="button" className={css.brand} onClick={() => startSession()} aria-label={t('session.new.label')}>
        ...
        <button type="button" className={css.iconButton} aria-label={t('session.new.label')} onClick={() => startSession()}>
