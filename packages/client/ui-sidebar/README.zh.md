...
Strugend 顶栏通过品牌插槽显示统一的琥珀色折带标志，并提供随应用打包的图片作为后备。历史抽屉保留已有工作区操作。
...
Strugend 构建使用常驻顶栏，提供历史、新建对话和设置。历史默认在停靠侧栏中打开，保留项目、分组和对话操作。选择或新建对话时侧栏保持打开。关闭按钮、历史开关或 Escape 可关闭侧栏；历史按钮可重新打开。关闭后焦点回到历史按钮。键盘导航可以离开侧栏，紧凑控件为较矮窗口中的对话留出空间。
