...
Strugend 对话默认隐藏系统提示、上下文注入和推理，用户启用诊断后才显示。已完成的过程行采用紧凑显示；用户消息、最终回答、错误和辅助审阅摘要保持可见。决策摘要关联持久化请求与结果，将所选标准显示为建议，并明确标出不可用或无定论的结果。标记为 `observedOnly` 的实验结果显示为仅记录。技术审阅数据仅在打开详情后挂载；摘要不宣称审阅改进或验证了结果。
...
输入框下方的用量计数包括当前智能体及可用的委派后代，弹窗分别显示两者的总量。普通对话分支不计入。打开弹窗会刷新目录元数据，但不会激活子智能体；缺少缓存投影时会显示部分统计，打开相应对话可重新构建其用量。这是包含重复缓存输入的已记录任务总量，并非服务商按 API 密钥统计的每日账单：辅助调用、未报告用量的请求及无关任务均不计入。
...
经典界面中，已完成轮次的操作页脚距前面的正文或扩展内容 20px。Strugend 的对话行间距为 12px，回答块间距为 10px，过程摘要与回答之间为 8px；页脚控件保留原有的点击区域。文本行高仍随字体大小偏好调整。
