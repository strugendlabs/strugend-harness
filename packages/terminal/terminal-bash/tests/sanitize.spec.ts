...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07strugend> ')).toEqual({ text: 'strugend> ', prompt: true, promptTail: 'strugend> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('strugend> ')).toEqual({ text: 'strugend> ', prompt: false, promptTail: 'strugend> ' })
  })
