...
Connection validation checks the selected profile's named key first. Profiles without `apiKeyEnv` use pi-ai's `checkAuth()`, preserving stored OAuth, ambient authentication, and explicitly anonymous custom routes without refreshing tokens. Unknown models still fail before admission. Remote authorization remains a request-time check.
