...
`ctx.llm.validateConnection(provider, model, signal?)` 在接受提示前检查所选适配器的本地认证要求。它不会发送模型请求，也不会刷新 OAuth。无认证要求的适配器继承空操作检查。成功表示本地配置存在，并不保证远程服务会接受它；每个流仍会解析当前凭据。`prepareCall()` 解析模型元数据并绑定分发，因此不属于认证检查。
