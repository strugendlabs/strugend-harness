...
it('serves the Web entry and assets without starting or contacting a Host', async () => {
  ...
  await writeFile(join(root, 'manifest.webmanifest'), '{"name":"Agent OS"}')
  const fetch = vi.fn()
  ...
  const manifest = await serveWebDocument(new Request('dsh-app://app/manifest.webmanifest'), root)
  expect(manifest.headers.get('content-type')).toBe('application/manifest+json')
  expect(await manifest.json()).toEqual({ name: 'Agent OS' })
