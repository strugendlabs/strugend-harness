...
The [Strugend development guide](../../AGENT_OS_README.md) owns the local fork’s launch and service configuration. Display branding uses Strugend Harness; native credential identity and existing data paths remain stable so saved keys, browser profiles, and chats remain accessible. The [intelligence implementation](../desktop-host/src/strugend-intelligence.ts) runs optional Laya checks concurrently with planning and tool execution without making the agent wait. Observe records judgments without changing Core context; Assist can add completed, fresh advice. An offline, checksum-pinned multilingual ONNX model ships under `runtime/laya`; one CPU worker serializes inference and unloads when idle. Local inference is disabled below 8 GiB RAM and under memory pressure. Automatic considers local fallback after Impossibl failure, subject to those limits. See the [resource policy and qualification limits](../../STRUGEND_PLAN.md). Graph memory is disabled and marked Coming soon. Task-owned delivery jobs build apps or publish verified commits to GitHub and Vercel, retaining receipts for recovery. Finder/Explorer actions use Electron’s native shell and report failures.

Local inference uses the same CPU runtime on Windows x64 and both Mac architectures. The dependency is pinned to ONNX Runtime 1.22.0 because [later npm archives omit Intel Mac bindings](https://github.com/microsoft/onnxruntime/issues/27961). Native qualification loads the packaged worker and weights before release.
