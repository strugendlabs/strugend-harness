...
Each turn receives a logged, bounded snapshot of local Memory and inventory counts; credentials are excluded. `read_soul` reads the full file and `update_soul` merges user-supplied preferences with revision conflict detection and local history. Agent writes refresh the Memory panel. Unavailable desktop context is reported without blocking unrelated tasks.

At login, `use_vault` checks metadata for the conversation's exact HTTPS site, opens the secure credential form when needed, or fills a saved login without returning its password. Agent fills require a fresh browser revision and respect takeover. Saving a login still requires the user's input in Vault; no password is inferred or requested in chat. `record_skill` starts a user demonstration, then returns scrubbed steps after stopping. `save_recorded_skill` creates a discoverable skill with task-specific catalog text; the filesystem provider watches for saved skills. These tools use ordinary persisted tool calls and results.

<a id="scheduled-tasks-and-updates"></a>
## Scheduled tasks and updates

Ask in chat to perform a task later or repeatedly, or open **Automations**. Save the task criteria, workspace, time zone and submission permission. Preview shows the next three occurrences. One-time dates include an explicit UTC offset; intervals are at least five minutes; calendar rules support daily, weekly, monthly and custom five-field cron schedules. Local times absent during a daylight-saving transition are skipped; a repeated local time runs once.

The desktop Host stores schedules and run history in a private SQLite database. One run executes at a time in a separate conversation, using the saved workspace, model and task mode. Foreground tasks and active jobs delay admission. Closing the window keeps enabled schedules running through the tray/menu bar; Quit stops execution. Start at login is an explicit preference; unsigned macOS previews may not register a login item, and the UI reports a setting that did not take effect. The computer must be awake and logged in: there is no hardware wake or cloud execution. Resume coalesces missed repeats into one latest occurrence within the saved catch-up limit (24 hours by default). A run normally has a 30-minute limit. Restarted in-flight work needs review and is not automatically retried.

Review permission allows research and local drafts while blocking shell commands, delegation and browser form interactions. Coding review runs use direct tools for this restriction. Automatic permission enables the saved actions with full access; the agent must use supplied facts and stop for missing access, personal answers or CAPTCHA. Application intents and confirmations share a durable ledger. Unconfirmed submissions require review; this cannot guarantee duplicate prevention on websites that provide no stable application identity or receipt. Each run records its outcome and links to its conversation. Pause cancels queued work; delete retains history; cancel stops an active run. Laya is optional and never required for scheduling.

Settings → Updates and the native **Check for Updates** action check released GitHub versions without downloading installers. Unsigned previews ask before downloading, verify the platform, byte count and SHA256, then offer the verified installer for manual installation. Quit Strugend before replacing the installed application. Unattended installation is unavailable for unsigned previews. Qualified signed distributions can opt into automatic downloads and installation while idle; the updater locks scheduler admission and rechecks active work before shutdown. An update does not split the Electron shell from its matching Host/runtime.
...
The base application carries Node.js and pnpm. `load_workspace_dependencies` returns their absolute paths immediately. Python, document libraries, Office skills and the native Office renderer belong to the optional Documents & data download; before installation the tool explains how to enable them in Settings → Optional tools. No Python interpreter starts during ordinary application startup.

Documents & data includes numpy, pandas, python-docx, python-pptx, openpyxl, Pillow, lxml and XlsxWriter with their locked dependencies. After installation, the tool returns interpreter, library and distribution-version paths; Desktop registers `office-docx`, `office-pptx` and `office-xlsx`. The skills reopen created files and run structural checks before delivery. See the [Office skill package](../../packages/skill/skill-office/README.md) for limitations.

Decision support and Documents & data use separate target-specific archives selected by a packaged catalog. Archives have pinned URLs, SHA256 hashes, compressed and expanded sizes and file counts. Downloads resume into private partial files; verification and extraction finish before an atomic publication under `$DSH_HOME/strugend-components/<component>/<content-version>`. Skipping either download leaves the coding agent usable. The optional native decision helper starts only when installed and resource admission permits it. Windows setup records an unchecked opt-in choice; macOS asks at first launch. Settings supports installation, cancellation and removal later.

The private Desktop `runtime/bin` directory is added only to package-installation processes, not the Host PATH inherited by PTC and agent shells. The dependency tool does not change PATH, environment variables or user package-manager configuration. Execute the returned pnpm script with the returned Node executable. Optional packs are qualified against the release; their version derives from content so unchanged packs remain reusable across application updates.

[The download lock](scripts/primary-runtime-lock.json) pins interpreter archives, Python distribution versions and target-specific wheel URLs and hashes. Native packaging verifies Office document read/write operations and dependency completeness before publishing the document archive. Both `dev:desktop` and `start:desktop` prepare only the Node/pnpm core runtime before launching Electron. Full release preparation creates optional archives outside the application payload. Unsigned preview installers enforce a 400 MiB maximum per core artifact; archive sizes remain visible in Optional tools. Native archive qualification reports installation states and phase durations, including Python document creation, PDF rendering and removal. Its ten-minute CI budget includes materializing and removing the complete optional packs on the runner filesystem.
...
Preview artifacts use `com.strugend.harness`, the Strugend client build profile, GitHub preview update discovery, no mandatory-update service, and explicit unsigned mode. They contain no provider keys or billing service. macOS previews are not notarized; signed production packaging retains its separate signing prerequisites. The [test preview notes](PREVIEW.md) describe installation and BYOK requirements. The [preview workflow](../../.github/workflows/strugend-preview.yml) builds all three native targets and publishes only after their checks pass. Pushing a newer release-family version on `codex/strugend-preview` triggers publication; ordinary code pushes do not. Manual dispatch remains available. Published artifacts include a platform-complete `strugend-update.json` with byte counts and SHA256 hashes.
...
The [packaged workflow fixture](../web/tests/strugend-desktop-smoke.mjs) uses an isolated profile and synthetic provider credentials. Its optional `--mock-keychain` flag avoids interactive macOS Keychain prompts for local unsigned builds; this mode does not qualify OS credential storage. Release CI runs without this flag.
...
Run `pnpm --dir apps/desktop run test:installer` from the repository root on an interactive Windows x64 desktop to build and exercise a small native test payload through the production installer configuration. Each run uses a unique product identity and sequentially exercises English-only and Chinese-only installer variants, selecting test labels from the displayed welcome button. Both variants install into private directories and uninstall after testing; screenshots and results remain under `.desktop-build/installer-tests/`. The checks include upgrades to registered paths with trailing separators, rejection of drive roots, and visible caption pixels in both light and dark themes. The optional `--signed` flag uses the Windows EV configuration below to sign test executables and the helper before embedding them; it does not enable an update feed.
