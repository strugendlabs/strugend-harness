...
每轮任务会收到已记录且有大小限制的本地记忆快照及库存数量，不含凭据。`read_soul` 读取完整文件，`update_soul` 合并用户提供的偏好并检查版本冲突、保留本地历史。智能体写入会刷新记忆面板。桌面上下文不可用时会报告状态，而不阻止无关任务。

登录时，`use_vault` 检查当前会话确切 HTTPS 网站的登录元数据，必要时打开安全凭据表单，或填充已保存的登录信息而不返回密码。智能体填充需要最新浏览器版本，并尊重用户接管。保存登录信息仍需用户在密码库中输入；不会推断密码或在聊天中索取。`record_skill` 开始用户演示并在停止后返回脱敏步骤。`save_recorded_skill` 使用任务相关的目录描述创建可发现的技能，文件系统提供者监视新保存的技能。这些工具使用常规持久化工具调用及结果。

<a id="scheduled-tasks-and-updates"></a>
## 定时任务和更新

在聊天中请求稍后或重复执行任务，或打开 **自动任务**。保存任务条件、工作区、时区和提交权限。预览显示接下来的三次运行时间。单次日期包含明确的 UTC 偏移；间隔至少五分钟；日历规则支持每天、每周、每月和自定义五字段 cron。夏令时跳变中不存在的本地时间会跳过；重复的本地时间只执行一次。

桌面 Host 将计划和运行历史存储在私有 SQLite 数据库中。每次只运行一个任务，在独立对话中使用保存的工作区、模型和任务模式。前台任务和活跃作业会推迟调度。关闭窗口后已启用的计划通过托盘或菜单栏继续运行；退出应用则停止。登录时启动需要明确选择；macOS 未签名预览版可能无法注册登录项，界面会报告未生效的选择。电脑必须唤醒并登录：不提供硬件唤醒或云执行。恢复时将错过的重复任务合并为补执行期限内（默认 24 小时）的最近一次。默认运行上限为 30 分钟。重启中断的任务需要审核，不会自动重试。

审核权限允许检索和本地草稿，但阻止 shell 命令、委派和浏览器表单交互。编程审核任务使用直接工具实现此限制。自动权限以完整访问执行保存的操作；代理必须使用提供的事实，并在缺少访问权限、个人答案或遇到验证码时停止。申请意图和确认共享持久化记录。未经确认的提交需要审核；若网站不提供稳定申请标识或回执，则无法保证避免重复。每次运行记录结果并链接到对话。暂停取消排队任务；删除保留历史；取消停止活跃运行。Laya 是可选组件，调度不依赖它。

设置 → 更新和原生 **检查更新** 操作检查 GitHub 已发布版本，但不下载安装程序。未签名预览版在下载前询问，验证平台、字节数和 SHA256，然后提供已验证的安装程序供手动安装。替换已安装应用前先退出 Strugend。未签名预览版不支持无人值守安装。合格的签名发行版可选择在空闲时自动下载和安装；更新器在关闭前锁定调度并再次检查活跃任务。更新始终保持 Electron 外壳与匹配的 Host 和运行时一致。
...
预览产物使用 `com.strugend.harness` 和 Strugend 客户端构建配置，提供 GitHub 预览版更新发现，关闭强制更新服务，并显式选择无签名模式。产物不含服务商密钥或计费服务。macOS 预览版未经公证；正式签名打包仍需满足独立的签名前提。[测试预览说明](PREVIEW.md) 介绍安装和 BYOK 要求。[预览工作流](../../.github/workflows/strugend-preview.yml) 构建三个本机目标，仅在所有检查通过后发布。在 `codex/strugend-preview` 上推送更新的发行族版本会触发发布；普通代码推送不会。也可手动触发。发布产物包含覆盖全部平台的 `strugend-update.json`，记录字节数和 SHA256。
...
Windows 安装程序使用原生 NSIS 页面，提供亮暗配色、系统阴影、可编辑的安装目录，以及默认勾选立即启动的完成页。安装仅面向当前用户。点击安装或按 Enter 均先规范化再校验当前路径，即使带有多余分隔符也会拒绝磁盘根目录；新安装位置必须为空，非空位置必须是已登记的安装目录。受影响安装路径中的程序运行时显示系统提示，并保持应用运行；其他目录中的同名应用不阻止安装。静默更新最多等待受影响应用退出十秒，若仍在运行则以退出码 2 结束。

可选下载和启动选项的文字使用所选主题的文字颜色；可选下载说明保留两行。主题在启动时跟随 Windows；可用 `/THEME=light`、`/THEME=dark` 和 `/THEME=auto` 显式选择配色。窗口在品牌控件准备完成后显示。欢迎页首次出现时，安装窗口会一次性移到普通窗口前方；若焦点在其他窗口，任务栏按钮会闪烁提示，但安装窗口不会始终置顶。进度读取锁定版本的 7-Zip 解压器百分比；目录替换、注册和清理仍使用有界估算。加权百分比不代表剩余时间。NSIS 报告成功后，进度条用 600 毫秒补满并短暂显示 100%，再显示完成页；切换目标时长为 750 毫秒。完成页保留窗口位置。点击完成后，安装程序先隐藏窗口，再启动已安装的可执行文件；启动失败会恢复页面以供重试。目录替换和失败恢复遵循上文描述的安装流程。首次启动的配置档案准备仍属于独立的 Desktop 操作。
