  ...
  ctx.tools.register(
    defineTool({
      ...
      parameters: {
        ...
        fields: { type: 'array', description: 'For fill_form: 1–30 distinct editable refs and replacement values from one observation. Fills sequentially and returns applied/remaining refs on a partial stop. Never replay an already-applied field blindly.', items: { type: 'object', properties: { ref: { type: 'string', required: true }, value: { type: 'string', required: true } }, additionalProperties: false } },
        paths: { type: 'array', items: { type: 'string' } },
        value: { description: 'For fill: complete replacement text, including newlines, for an observed editable ref. Maximum 20000 characters. Native date/time fields use ISO format (date: YYYY-MM-DD); dropdowns accept an exact observed option value or label.', type: 'string' },
      ...
      output: {
        ...
        render: (_args, value) => {
          const result = value as unknown as {
            observation: BrowserObservation | BrowserTabList
            image?: ImageAttachmentRef
            visualWarning?: string
          }
          const content: ContentBlock[] = [{ type: 'text', text: JSON.stringify({ ...result.observation, ...(result.visualWarning ? { visualWarning: result.visualWarning } : {}) }) }]
      ...
      execute: async (args, execution) => {
        ...
        const { provider, model } = execution.agent.session.requestHeader()?.config ?? ctx.agentDefaultModel.currentSelection()
        const visual = (await ctx.llm.resolveModelInfo(provider, model)).inputModalities?.includes('image') === true
        const requested = command.action === 'observe' && command.screenshot === undefined && visual
          ? { ...command, screenshot: true } : command
        const request = (value: BrowserAction) => desktopRequest<BrowserObservation | BrowserTabList>({
          method: 'browser', sessionId: owner.session.id,
          workspace: owner.session.header.cwd, command: value,
        }, execution.signal)
        let observation: BrowserObservation | BrowserTabList
        let visualWarning: string | undefined
        try { observation = await request(requested) } catch (error) {
          execution.signal.throwIfAborted()
          if (requested.action !== 'observe' || requested.screenshot !== true) throw error
          observation = await request({ ...requested, screenshot: false })
          visualWarning = `Screenshot unavailable; use these current page controls. ${error instanceof Error ? error.message : String(error)}`
        }
        if (visual && 'state' in observation && ['open', 'click', 'click_point', 'back', 'forward', 'reload', 'scroll'].includes(command.action))
          try {
            observation = await desktopRequest<BrowserObservation>({ method: 'browser', sessionId: execution.agent.session.id,
              command: { action: 'observe', tabId: observation.state.tabId, screenshot: true } }, execution.signal)
          } catch (error) {
            execution.signal.throwIfAborted()
        ...
        return JSON.parse(JSON.stringify({ observation, image, visualWarning })) as Record<string, JsonValue>
      },
      presentCall: args => ({
        card: 'generic',
